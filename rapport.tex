...
\cite{qccompleteness}
% intro (etat de l'art rapide & motivation)
...
Et les $\mathcal E_0$ sont les égalités (6) à (10) de la Figure 1
...
Pour les $\pro{\Sigma}$, on a le système défini en Figure 2
...
\begin{example}
    ...
    Alors on obtient après application des règles:
...
Pour les $\prop{\varnothing}$, on a le système composé des règles (R1) jusqu'à (R9) de la Figure 2, ainsi que celles définies en Figure 3
...
Durant mon stage, j'ai implémenté l'algorithme produisant les certifiats d'équivalence pour les deux cas déjà traités par mon encadrante (les $\pro{\Sigma}$ et les permutations $\prop{\varnothing}$), et j'ai fait des benchmarks pour avoir une idée de leur efficacité.

J'ai aussi trouvé un système de réécriture qui pourrait étendre les deux cas précédant aux $\prop{\Sigma}$, mais je n'ai pas eu le temps durant mon stage de formaliser ce dernier en Isabelle.
...
\begin{remark}
    ...
    On considèrera donc pendant toute cette partie que les circuits sur lesquels on travaille sont déjà séparé en couches.
\end{remark}
...
\begin{definition}
    ...
    \input{pic/swap_gates.tikz} où $i,j$ sont les priorités des fils sortants et les portes $X$ et $Y$ sont à la même profondeur $p$

    \input{pic/through_perm1.tikz} où $\sigma$ est une permutation
    
    \input{pic/through_perm2.tikz} où $\sigma$ est une permutation
...
\begin{proof}
    ...
    On remarque que l'application de la règle 2 ou de la règle 3 fait décroître $n$, et que l'application de la règle 1 ne change pas $n$ et faite décroître $m$.
...
TODO papier original, papier quantique, papier bench, papier graphe
